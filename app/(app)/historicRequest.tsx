...
const STATUS_MAP = {
  ...
  "coletado": { label: "Coletado", color: "#4CAF50", icon: { lib: Feather, name: "check" } },
  "cancelado": { label: "Cancelado", color: "#F44336", icon: { lib: Feather, name: "x" } },
  
  // para manter compativel com o antigo
  "1": { label: "Pendente", color: "#FFC107", icon: { lib: Feather, name: "clock" } },
  "2": { label: "Aceito", color: "#2196F3", icon: { lib: Feather, name: "check-circle" } },
  "3": { label: "Coletado", color: "#4CAF50", icon: { lib: Feather, name: "check" } },
  "4": { label: "Cancelado", color: "#F44336", icon: { lib: Feather, name: "x" } },
};
...
export default function HistoricoColetasScreen() {
  ...
  const fetchColetas = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/coletas?id_clientes=${user.client_id}`);
      ...
      // Ordenar primeiro por prioridade de status, depois por data de criação
        const getPrioridade = (status) => {
          if (!status) return 99;
          const prioridades = {
            "coletado": 1,     // Coletado (maior prioridade para mostrar primeiro)
            "aceitado": 2,     // Aceito
            "pendente": 3,     // Pendente
            "cancelado": 4,    // Cancelado (menor prioridade)
  ...
  const handleCancelarColeta = async () => {
    ...
    if (selectedColeta.solicitacao.estado_solicitacao !== "pendente") {
      Alert.alert(
    ...
    try {
      ...
      await axios.patch(
        ...
        { estado_solicitacao: "cancelado" }  // Cancelado
      );
