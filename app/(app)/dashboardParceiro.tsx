...
const STATUS_MAP = {
  ...
  "coletado": { label: "Coletado", color: "#4CAF50", icon: "check" },
  "cancelado": { label: "Cancelado", color: "#F44336", icon: "x-circle" },
  
  // Manter compatibilidade
  "1": { label: "Pendente", color: "#FFC107", icon: "clock" },
  "2": { label: "Aceito", color: "#2196F3", icon: "check-circle" },
  "3": { label: "Coletado", color: "#4CAF50", icon: "check" },
  "4": { label: "Cancelado", color: "#F44336", icon: "x-circle" },
};
...
export default function DashboardPedidosScreen() {
  ...
  const fetchPedidos = async () => {
    try {
      ...
      const coletasResponse = await axios.get(`${API_BASE_URL}/coletas?id_parceiros=${user.partner_id}`);
      ...
      const pedidosOrdenados = pedidosValidos.sort((a, b) => {
        ...
        const prioridadeStatus = {
          ...
          // Manter compatibilidade com números antigos
          "2": 1,
          "1": 2,
          "3": 3, 
          "4": 4,
  ...
  const formatarData = (dataString) => {
    ...
    const handleAtualizarStatus = async (novoStatus) => {
      ...
      try {
        // Mapear os status para as novas strings
        let statusString;
        switch (novoStatus) {
          case "3":
          case "coletado":
            statusString = "coletado";
            break;
          case "5":
          case "cancelado":
            ...
            break;
          default:
            statusString = novoStatus;
        }
        
        // Atualizar o status da solicitação
        await axios.patch(`${API_BASE_URL}/solicitacoes/${selectedPedido.solicitacao.id}`, {
          estado_solicitacao: statusString
        });
        
        // Se o status for "coletado", atualizar a data de finalização
        if (statusString === "coletado") {
          await axios.patch(`${API_BASE_URL}/solicitacoes/${selectedPedido.solicitacao.id}`, {
          ...
          <View style={styles.modalContent}>
            ...
            {selectedPedido && (
              <ScrollView style={styles.modalBody}>
                ...
                {selectedPedido.solicitacao?.estado_solicitacao === "aceitado" && (
                  <TouchableOpacity
                    style={styles.acaoButton}
                    onPress={() => handleAtualizarStatus("coletado")}
                ...
                {["pendente", "aceitado"].includes(selectedPedido.solicitacao?.estado_solicitacao) && (
                  <TouchableOpacity
                    style={styles.cancelarButton}
                    onPress={() => handleAtualizarStatus("cancelado")}
                    disabled={atualizandoStatus}
                  >
                    {atualizandoStatus ? (
                      <ActivityIndicator size="small" color="#FFFFFF" />
